...
class SuspensionProvider with ChangeNotifier {
  ...
  Future<void> deleteItem(String id, BuildContext context) async {
    await ProviderModel().deleteItem(context, id, _items, 'suspension');
    notifyListeners();
  }
  ...
  Future<void> undoDelete(
    ...
    await ProviderModel()
        .undoDelete(context, index, deletedItem, _items, 'suspension');
  ...
  Future<void> addItem(ItemModel itemModel, BuildContext context) async {
    await ProviderModel().addItem(itemModel, context, _items, 'suspension');
    notifyListeners();
  }

  Future<void> fetchItems(BuildContext context) async {
    const url =
        'https://zfr-inventory.firebaseio.com/department/suspension.json';
    try {
