...
class ElectronicsProvider with ChangeNotifier {
  ...
  Future<void> undoDelete(
    ...
    await ProviderModel(token)
        .undoDelete(context, index, deletedItem, _items, 'elctronics');
  ...
  Future<void> addItem(ItemModel itemModel, BuildContext context) async {
    await ProviderModel(token)
        .addItem(itemModel, context, _items, 'electronics');
    notifyListeners();
  }

  Future<void> fetchItems(BuildContext context) async {
    final url =
        'https://zfr-inventory.firebaseio.com/department/electronics.json?auth=$token';
    try {
