...
class SteeringProvider with ChangeNotifier {
 ...
 List<double> get va {
  ...
  Future<void> deleteItem(String id, BuildContext context) async {
    await ProviderModel().deleteItem(context, id, _items, 'steering');
    notifyListeners();
  }

  Future<void> undoDelete(BuildContext context,int index, ItemModel deletedItem) async{
   await ProviderModel().undoDelete(context,index, deletedItem, _items,'steering');
  ...
  Future<void> addItem(ItemModel itemModel, BuildContext context) async {
    await ProviderModel().addItem(itemModel, context, _items, 'steering');
    notifyListeners();
  }


  Future<void> fetchItems(BuildContext context) async {
    const url = 'https://zfr-inventory.firebaseio.com/department/steering.json';
    try {
