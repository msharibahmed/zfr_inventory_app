...
class ExhaustProvider with ChangeNotifier {
  ...
  Future<void> deleteItem(String id, BuildContext context) async {
    await ProviderModel().deleteItem(context, id, _items, 'exhaust');
    notifyListeners();
  }

  Future<void> undoDelete(BuildContext context,int index, ItemModel deletedItem) async{
   await ProviderModel().undoDelete(context,index, deletedItem, _items,'exhaust');
  ...
  Future<void> addItem(ItemModel itemModel, BuildContext context) async {
    await ProviderModel().addItem(itemModel, context, _items, 'exhaust');
    notifyListeners();
  }


  Future<void> fetchItems(BuildContext context) async {
    const url = 'https://zfr-inventory.firebaseio.com/department/exhaust.json';
    try {
